...
raw_data = pd.read_csv('datasets/ApartmentPrice_SG_Vietnam_June2021.csv')
...
raw_data["TOTAL NUMBER OF BEDROOMS"] = raw_data["NUMBER OF BEDROOMS"] + raw_data["NUMBER OF TOILETS"] 
corr_matrix = raw_data.corr(numeric_only=True)
print(corr_matrix["PRICE IN MILLION VND"].sort_values(ascending=False)) # print correlation b/w a feature and other features
raw_data.drop(columns = ["AREA PER ROOM", "TOTAL NUMBER OF BEDROOMS"], inplace=True) # remove experiment columns
#endregion
